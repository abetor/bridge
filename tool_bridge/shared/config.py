"""Fail-closed TOML loading where an unknown field is an error.

Silently accepting a misspelled field creates configuration that appears to
apply but does nothing. Rejecting unknown fields prevents that failure mode.
"""
...
class CommonConfigError(ValueError):
    """The shared config.toml exists but does not match schema v1."""
...
@dataclass(frozen=True)
class CommonConfig:
    """Shared configuration result; exists distinguishes absence from empty data."""
...
def load_toml(path: str | Path, *, known: set[str],
    ...
    ValueError covers invalid syntax, unknown fields, and missing required
    fields. Defaults and value typing belong to the caller; this layer only
    enforces the agreed field set.
    """
    path = Path(path)
...
def tools_data_root() -> Path:
    """Return the shared data root from TOOLS_DATA or the documented default."""
...
def _absolute_path(value: object, *, path: Path, key: str) -> Path:
    ...
    if not result.is_absolute():
        raise _error(path, key, "path must be absolute (~ is allowed)")
    return result
...
def _env_names(value: object, *, path: Path, key: str) -> tuple[str, ...]:
    if not isinstance(value, list):
        raise _error(path, key, "expected a list of variable names")
    names = tuple(_text(item, path=path, key=f"{key}[{i}]")
                  for i, item in enumerate(value))
    if len(set(names)) != len(names):
        raise _error(path, key, "variable names must be unique")
    for i, name in enumerate(names):
        if not _ENV_NAME.fullmatch(name):
            raise _error(path, f"{key}[{i}]", "expected an environment variable name, not a value")
    return names
...
def load_common_config(root: str | Path | None = None) -> CommonConfig:
    ...
    A missing file is valid and yields ``CommonConfig.exists=False``. Any file
    that exists but cannot be read or validated raises CommonConfigError with
    the full path and schema key. Configuration is never applied partially.
    """
    ...
    try:
        ...
        raise _error(path, "TOML", f"file is not readable: {e}") from e
    ...
    for name, value in sorted(raw_harnesses.items()):
        if name not in _HARNESS_NAMES:
            raise _error(path, f"harness.{name}",
                         f"unknown harness; expected one of {sorted(_HARNESS_NAMES)}")
        key = f"harness.{name}"
